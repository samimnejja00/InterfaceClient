...
const EXTERNAL_CHATBOT_API_URL = "http://localhost:3001/api/chat/message";

export default function PublicChatbot() {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState([
    { from: "bot", text: "Bonjour ! Je suis l'assistant COMAR. Je peux vous renseigner sur nos offres d'assurance et vous aider à naviguer sur la plateforme. Comment puis-je vous aider ?" }
  ]);
  ...
  const sendMessageText = async (rawText, { clearInput = false } = {}) => {
    ...
    try {
      const res = await fetch(EXTERNAL_CHATBOT_API_URL, {
        method: "POST",
      ...
      setMessages((prev) => [
        ...prev,
        { from: "bot", text: error?.message || "Erreur de connexion au serveur de chat." },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      {/* Toggle Button */}
      <button
        className="w-14 h-14 rounded-full bg-comar-royal text-white shadow-lg hover:bg-comar-green-dark hover:shadow-xl transition-all duration-300 flex items-center justify-center text-xl hover:scale-105 active:scale-95"
        onClick={toggleChat}
      >
        {isOpen ? "✕" : "💬"}
      </button>
      ...
      >
        ...
        <div className="shrink-0 p-3 bg-white border-t border-gray-100 flex items-center gap-2">
          ...
          />
          <button
            className="px-4 py-2.5 bg-comar-royal text-white text-sm font-semibold rounded-xl hover:bg-comar-green-dark transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
            onClick={sendMessage}
